...
const SERVICES = [
  ...
  { url: 'https://apps.fcctp.edu.pe/bienestar-universitario', name: 'Bienestar Universitario' },
...
async function checkService(service) {
  ...
  try {
    ...
    const latency = Date.now() - start;
    return { isUp: false, latency, statusCode: null, error: err.code || err.message };
  }
}
...
async function sendEmail(transporter, subject, html) {
  ...
  try {
    await transporter.sendMail({ from: `FCCTP Status <${from}>`, to, subject, html });
...
function buildSummaryEmail(changes, downServices, isReminder) {
  ...
  if (changes.length > 0 && !isReminder) {
    ...
    for (const c of changes) {
      const statusLabel = c.status === 'down' ? '<span style="color: #dc2626; font-weight: bold;">CAÍDO</span>' : '<span style="color: #16a34a; font-weight: bold;">RECUPERADO</span>';
      const detail = c.status === 'down' ? `(Error: ${c.error || 'Sin respuesta'})` : `(Inactividad: ${c.duration} min)`;
...
async function main() {
  ...
  for (const service of SERVICES) {
    const prev = statusData[service.url] || { status: 'up', last_state_change: lima.iso, last_notification_sent: null };
    ...
    if (currentStatus !== prev.status) {
      ...
      if (currentStatus === 'down') {
        ...
        const openIncident = incidentsData.incidents.find(i => i.url === service.url && !i.resolved_at);
  ...
  if (currentHour === 7 && downServices.length > 0) {
    const alreadySentToday = downServices.some(s => {
      const lastNotif = s.last_notification_sent ? new Date(s.last_notification_sent) : null;
  ...
  latencyData.checks = latencyData.checks.filter(c => new Date(c.timestamp).getTime() > twentyFourHoursAgo);
